...
#include <pcap.h>      // For live capture (pcap_open_live, pcap_next_ex, etc.)
#include <csignal>     // For graceful Ctrl+C shutdown

using namespace PacketAnalyzer;
using namespace DPI;

// Global shutdown flag for live capture (set by SIGINT/SIGTERM)
...
public:
  ...
  bool isBlocked(uint32_t src_ip, AppType app, const std::string& sni) const {
    ...
    // Block by Domain (substring match - case insensitive)
    if (!sni.empty()) {
        ...
        for (const auto& dom : blocked_domains_) {
            std::string lower_dom = dom;
            std::transform(lower_dom.begin(), lower_dom.end(), lower_dom.begin(), ::tolower);
            
            if (lower_sni.find(lower_dom) != std::string::npos) {
                return true;
            }
        }
    }
...
private:
    ...
    auto sni_opt = SNIExtractor::extract(payload, pkt.payload_length);
    if (sni_opt) {
        std::string sni = *sni_opt;
        flow.sni = sni;
        flow.app_type = sniToAppType(sni);
        flow.classified = true;
        
        // DEBUG PRINT - This will show you exactly what is being seen
        std::cout << "[SNI Detected] " << sni 
                  << " | App: " << appTypeToString(flow.app_type) 
                  << " | SrcPort: " << pkt.tuple.src_port 
                  << "\n";
        return;
    }
}
...
public:
    ...
    DPIEngine(const Config& cfg) : config_(cfg) {
        ...
        std::cout << "\n";
        std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
        std::cout << "║              DPI ENGINE v2.1 (Multi-threaded + Live Capture)  ║\n";
        std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    ...
    // Updated signature: source can be PCAP file OR interface name when is_live=true
    bool process(const std::string& source, const std::string& output_file, bool is_live = false);
...
bool DPIEngine::process(const std::string& source, const std::string& output_file, bool is_live) {
    // Open output PCAP
    ...
    // === Open capture source (file OR live interface) ===
    PcapReader reader;
    ...
    if (!is_live) {
        ...
        live_handle = pcap_open_live(source.c_str(), 65536, 1 /*promisc*/, 1000 /*timeout ms*/, errbuf);
        if (!live_handle) {
            std::cerr << "[Live Capture] Failed to open interface '" << source << "': " << errbuf << "\n";
            output.close();
            return false;
        }
        std::cout << "[Live Capture] Successfully opened interface: " << source << "\n";
    }
    ...
    // Start all threads (same pipeline for both modes)
    ...
    // === Packet processing loop (common lambda to avoid duplication) ===
    std::cout << (is_live ? "[Live Capture] Starting real-time capture on " + source + " (Ctrl+C to stop)..." 
                          : "[Reader] Processing packets...") << "\n";

    uint32_t pkt_id = 0;

    // IP string -> uint32_t parser (moved out of loop)
    ...
    // Core packet handler - feeds directly into LB -> FP pipeline
    auto process_packet = [&](RawPacket r) {
        ParsedPacket parsed;
        if (!PacketParser::parse(r, parsed)) return;
        if (!parsed.has_ip || (!parsed.has_tcp && !parsed.has_udp)) return;

        Packet pkt;
        pkt.id = pkt_id++;
        pkt.ts_sec = r.header.ts_sec;
        pkt.ts_usec = r.header.ts_usec;
        pkt.tcp_flags = parsed.tcp_flags;
        pkt.data = std::move(r.data);
        ...
        // Calculate payload offset/length (same logic as before)
        ...
        // Dispatch to Load Balancer (same hash-based routing)
        FiveTupleHash hasher;
    ...
    RawPacket raw;  // reused for both modes
    if (!is_live) {
        while (reader.readNextPacket(raw)) {
            process_packet(std::move(raw));
        }
        ...
        while (!g_shutdown && (res = pcap_next_ex(live_handle, &pkthdr, &packet_data)) >= 0) {
            if (res == 0) continue;  // timeout - no packet

            // Fill RawPacket exactly as PcapReader does for files
            raw.header.ts_sec = pkthdr->ts.tv_sec;
            raw.header.ts_usec = pkthdr->ts.tv_usec;
            raw.header.incl_len = pkthdr->caplen;
            raw.header.orig_len = pkthdr->len;
            raw.data.assign(packet_data, packet_data + pkthdr->caplen);
        ...
        if (g_shutdown) {
            std::cout << "[Live Capture] Stopped. Processed " << pkt_id << " packets\n";
        } else if (res == -1) {
            std::cerr << "[Live Capture] Error: " << pcap_geterr(live_handle) << "\n";
        }
    }
...
void printUsage(const char* prog) {
    std::cout << R"(
DPI Engine v2.1 - Multi-threaded Deep Packet Inspection + Real-Time Capture
===========================================================================

Usage: )" << prog << R"( <input> <output.pcap> [options]

<input> can be:
  - A .pcap file (offline mode)
  - Network interface name when using --live (real-time mode)
...
Options:
  ...
  --live <interface>     Real-time capture from network interface (e.g. eth0, enp0s3, any)

Examples:
  Offline:   )" << prog << R"( capture.pcap filtered.pcap --block-app YouTube --block-ip 192.168.1.50
  Live:      )" << prog << R"( eth0 filtered.pcap --live eth0 --block-domain youtube.com
)";
}
...
int main(int argc, char* argv[]) {
    ...
    if (live_mode && live_interface.empty()) {
    ...
    // Setup signal handler for graceful live capture shutdown
    ...
    // source = PCAP file OR interface name
    std::string source = live_mode ? live_interface : input;
    
    if (!engine.process(source, output, live_mode)) {
        return 1;
    }
